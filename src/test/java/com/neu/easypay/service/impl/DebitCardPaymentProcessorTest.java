...
class DebitCardPaymentProcessorTest {

  private DebitCardPaymentProcessor processor;
  ...
  void setUp() {
    ...
    processor = DebitCardPaymentProcessor.builder()
  ...
  @Test
  void processPayment_largeAmount_fail() {
    UserAccount userAccount = UserAccount.builder()
        .balance(20.0)
        .build();
    when(userAccountMapper.getByUserUUID("user1")).thenReturn(userAccount);
    ...
    CardInfo cardInfo = CardInfo.builder()
        ...
        .cardHolderZipcode("98101")
        .build();
    PaymentProcessingResult result = processor.processPayment("payer1", cardInfo, "user1", 100000.0);
  ...
  @Test
  void getPaymentType() {
    assertEquals("DEBIT", processor.getPaymentType());
  }
}